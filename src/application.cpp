...
namespace gigno {
	...
	int giApplication::run() {
		...
		RenderedEntity first{ m_Vertices, m_Indices, glm::vec3{1.f, 0.0f, 0.0f } };
		first.Transform.translation = glm::vec3{ 0.0f, 0.0f, 0.0f };
		first.Transform.scale = glm::vec3{ 1.0f, 1.0f, 1.0f };
		first.Transform.rotation = glm::vec3(0.0f, 0.0f, 0.0f);
		float rot = 0;
		float y = -5.0f;

		RenderedEntity second{ m_Vertices, m_Indices, glm::vec3{0.0f, 1.0f, 0.0f} };
		second.Transform.translation = glm::vec3{ 2.0f, 0.0f, 1.0f };
		second.Transform.rotation = glm::vec3(0.0f, glm::radians(30.0f), 0.0f);

		DomeCamera camera(20000.0f);
		...
		camera.SetTarget(first.Transform.translation);
		...
		while (!m_RenderingServer.WindowShouldClose()) {
			...
			m_EntityServer.Tick(deltaTime.count() / 1000.0);
