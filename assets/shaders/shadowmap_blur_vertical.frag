...
void main() {
    ...
    float factor = (SCALE/SHADOW_MAP_HEIGHT);

    color += texture(image, coord + factor * vec2(0, -3)).xy * (1.0/64.0);
    color += texture(image, coord + factor * vec2(0, -2)).xy * (6.0/64.0);
    color += texture(image, coord + factor * vec2(0, -1)).xy * (15.0/64.0);
